...
CREATE INDEX `auth_sessions_retention_idx` ON `auth_sessions` (`expires_at`,`revoked_at`);
...
CREATE TABLE `login_attempts` (
  ...
  `attempted_at` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL
);
CREATE INDEX `login_attempts_login_code_idx` ON `login_attempts` (`login_code`,`succeeded`,`attempted_at`);
CREATE INDEX `login_attempts_ip_idx` ON `login_attempts` (`ip_hash`,`succeeded`,`attempted_at`);
CREATE INDEX `login_attempts_retention_idx` ON `login_attempts` (`attempted_at`);
...
CREATE TABLE `biometric_verifications` (
  ...
  `verified_at` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
