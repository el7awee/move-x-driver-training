...
const values:Record<string,string>={morning:"صباحية",evening:"مسائية",night:"ليلية",flexible:"مرنة",active:"نشط",vacation:"إجازة",suspended:"موقوف",resigned:"مستقيل",terminated:"منتهي الخدمة",valid:"سارية",expiring:"قاربت الانتهاء",expired:"منتهية",pending:"قيد المراجعة",rejected:"مرفوضة",not_provided:"غير مقدمة",negative:"سلبي",positive:"إيجابي"};
...
export default async function SupervisorPage(){
  ...
  return <OperationalShell role={current.user.role} name={current.user.displayName}>
    ...
    <section className="op-panel"><h2>دليل الكباتن وحالة المستندات</h2><p className="op-muted">لا تعرض هذه الشاشة الرقم القومي أو صور البطاقة أو الصحيفة أو تحليل المخدرات.</p><div className="op-table-wrap"><table><thead><tr><th>الكابتن</th><th>الهاتف</th><th>الوردية</th><th>الحالة</th><th>الرخصة</th><th>الصحيفة</th><th>تحليل المخدرات</th><th>عدد المستندات</th></tr></thead><tbody>{drivers.map((driver,index)=><tr key={String(driver.id??index)}><td>{String(driver.full_name)}<br/><small>{String(driver.driver_code)}</small></td><td>{String(driver.phone)}</td><td>{values[String(driver.primary_shift)]??String(driver.primary_shift)}</td><td>{values[String(driver.employment_status)]??String(driver.employment_status)}</td><td>{values[String(driver.driving_license_status)]??String(driver.driving_license_status)} — {String(driver.driving_license_expiry??"—")}</td><td>{values[String(driver.criminal_record_status)]??String(driver.criminal_record_status)} — {String(driver.criminal_record_expiry??"—")}</td><td>{values[String(driver.drug_test_status)]??String(driver.drug_test_status)} — {String(driver.drug_test_expiry??"—")}</td><td>{String(driver.document_count)}</td></tr>)}</tbody></table></div></section>
