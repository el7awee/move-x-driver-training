...
export function operationalError(error: unknown) {
  ...
  if (message.includes("invalid_document_size")) return Response.json({error:"يجب ألا يتجاوز الملف 8 ميجابايت.",code:"invalid_document_size"},{status:400});
  if (message.includes("invalid_document_mime")||message.includes("invalid_document_type")) return Response.json({error:"نوع الملف غير مسموح أو لا يطابق محتواه الحقيقي.",code:"invalid_document"},{status:400});
  if (message.includes("driver_data_protection_unavailable")) return Response.json({error:"حماية البيانات الحساسة غير مضبوطة على هذه البيئة.",code:"data_protection_unavailable"},{status:503});
  if (message.includes("document_storage_unavailable")) return Response.json({error:"رفع المستندات متوقف حتى يكتمل إعداد Google Drive الخاص.",code:"document_storage_unavailable"},{status:503});
  if (message.includes("document_storage_")) return Response.json({error:"تعذر الاتصال بمخزن المستندات الخاص.",code:"document_storage_error"},{status:502});
  ...
  if (message.includes("user_not_found")||message.includes("vehicle_not_found")||message.includes("assignment_not_found")||message.includes("document_not_found")) return Response.json({error:"السجل المطلوب غير موجود.",code:"not_found"},{status:404});
