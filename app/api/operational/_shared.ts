...
export function operationalError(error: unknown) {
  ...
  if (message.includes("invalid_user")||message.includes("invalid_vehicle")||message.includes("invalid_assignment")||message.includes("invalid_handover")||message.includes("invalid_settings")) return Response.json({error:"البيانات المدخلة غير صالحة.",code:"invalid_input"},{status:400});
